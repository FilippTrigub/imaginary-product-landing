...
(function () {
    ...
    const testimonials = [
        ...
        { name: 'Fatima Al-Rashid', role: 'Architect', text: 'Being able to walk through my building models in real scale is simply incredible.' }
    ];
    ...
    // ── Scheduled Dummy Notifications ────────────────────────────────
    var messages = [
        '🚀 New visitor from ' + stats.topReferrer + '!',
        '📊 ' + stats.visitors + ' visitors today so far.',
        '⭐ "' + testimonials[randomInt(0, testimonials.length - 1)].text.slice(0, 60) + '…"',
        '🔔 NovaSphere Pro plan is trending!',
        '🌐 Session avg: ' + stats.avgSessionDuration
    ];

    // Show the first toast after 2 s, then one every 8 s
    var msgIndex = 0;
    setTimeout(function tick() {
        if (msgIndex < messages.length) {
            showToast(messages[msgIndex]);
            msgIndex++;
    ...
    window.NovaDummy = {
        ...
        generateSessionStats: generateSessionStats,
        showToast: showToast
    };

    console.log('ℹ️  window.NovaDummy is available — try NovaDummy.showToast("Hello!")');
})();
