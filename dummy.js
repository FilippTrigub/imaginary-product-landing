...
(function () {
    ...
    var notificationTemplates = [
        'New sign-up from {city}!',
        '{count} users are viewing the pricing page right now.',
        'NovaSphere Pro just sold out in {city}.',
        'A new review was posted: "{stars} stars — amazing product!"',
        '{name} just upgraded to the Enterprise plan.'
    ];

    var cities = ['San Francisco', 'Tokyo', 'Berlin', 'London', 'Sydney', 'Toronto', 'Seoul', 'Mumbai'];
    var names = ['Alex', 'Jordan', 'Sam', 'Taylor', 'Morgan', 'Casey', 'Riley', 'Quinn'];

    function generateNotification() {
        var template = pick(notificationTemplates);
        return template
            .replace('{city}', pick(cities))
            .replace('{count}', randomInt(3, 42))
            .replace('{stars}', randomInt(4, 5))
    ...
    // Attach to window.NovaSphere namespace when running in a browser
    if (typeof window !== 'undefined') {
        window.NovaSphere = window.NovaSphere || {};
        window.NovaSphere.Dummy = DummyAPI;

        // Auto-print dashboard on load
        if (document.readyState === 'loading') {
            document.addEventListener('DOMContentLoaded', printDashboard);
        } else {
            printDashboard();
        }
    }

    // Support Node.js / CommonJS for quick testing
