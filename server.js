...
function addBannerToHtml(content, isHeaderPage = false) {
  if (foobar) {
    const bannerHtml = `
      <div id="env-banner" style="background-color: #ff6b6b; color: white; text-align: center; padding: 14px 20px; font-weight: bold; position: fixed; top: 0; left: 0; width: 100%; z-index: 1001; font-size: 1rem; letter-spacing: 0.5px; box-shadow: 0 2px 8px rgba(0,0,0,0.15);">
...
// Route for the main page (and header.html directly)
...
// Serve static files (after HTML routes so banner injection takes priority)
